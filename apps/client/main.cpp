...
void defineOptimizerOptions(FixedVars* varsPtr, po::options_description* odptr){
    odptr->add_options()
            ...
            ("security,k",
             po::value < unsigned
    int > (&varsPtr->k)->default_value(DEFAULT_SECURITY),
            "Security bits wanted")

    ("dmin",
            po::value < unsigned
    int > (&varsPtr->dMin)->default_value(DEFAULT_DMIN),
            "Min dimension value to test")

    ("dmax",
            po::value < unsigned
    int > (&varsPtr->dMax)->default_value(DEFAULT_DMAX),
            "Max dimension value to test")

    ("alphaMax,a",
            po::value < unsigned
    int > (&varsPtr->alphaMax)->default_value(DEFAULT_ALPHAMAX),
...
void processOptions(FixedVars* varsPtr, ClientParams* paramsPtr, po::variables_map vm){
    ...
    if(vm.count("dry-run"))
    {
        ...
        std::cout << "CLI: Setting default values for dry-run mode (a thousand mp3 files, 100MBit/s connection)" <<
        std::endl;
    }
 ...
 << std::endl;
    ...
    if (vm.count("crypto-params"))
    {
        ...
        if (fields.size() > 4) {
            std::cout << "CLI: WARNING Absorption size will be overriden by the optimizer" <<
            varsPtr->manual_crypto_params << std::endl;
    ...
    if (vm.count("alphaMax")) {
        varsPtr->alphaMax = vm["alphaMax"].as < unsigned
        int > ();
    ...
    // Hidden options
    if (vm.count("reclvl")) {
        varsPtr->dMax = vm["reclvl"].as < unsigned
        int > ();
        varsPtr->dMin = vm["reclvl"].as < unsigned
        int > ();
...
int main(int argc, char **argv)
{
    ...
    cout << "waiting for fingerprint validation" << endl;
    if (validateFingerprint()) {
        ...
        cout << "Fingerprint does not verify" << endl;
    }
